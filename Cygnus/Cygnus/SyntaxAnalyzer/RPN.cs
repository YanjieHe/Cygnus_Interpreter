...
namespace Cygnus.SyntaxAnalyzer
{
    public sealed class RPN
    {
               ...
               DisplayStack(op);
                switch (item.tokenType)
                {
                    ...
                    case TokenType.Dot:
                        {
                            ...
                            while (op.Count > 0)
                            {
                                var op_peek = GetOp(op.Peek());
                                if (op_peek.IsBraket()) break;
                                int cmp = Priority(current) - Priority(op_peek);
                                if (cmp > 0) break;
                                else if (cmp == 0)
                                {
                    ...
                    case TokenType.LeftBracket:
                        while (op.Count > 0)
                        {
                            if (!GetOp(op.Peek()).IsBraket() && GetOp(op.Peek())!= Operator.Call)
                                Operands.Add(op.Pop());
                        }
                        op.Push(item); break;
                    ...
                    case TokenType.RightBrace:
                        FindBraket(op, Operator.RightBrace);
                        break;
                    case TokenType.RightBracket:
                        FindBraket(op, Operator.RightBracket);
                        break;
                    default:
        ...
        public void FindBraket(Stack<Lexeme> op, Operator braket)
        {
            while (op.Count > 0)
            {
                var current = op.Pop();
                if (GetOp(current).IsMatch(braket))
                {
            ...
            throw new SyntaxException("There are mismatched " + braket);
        }
